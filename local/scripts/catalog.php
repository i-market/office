...
foreach($arResult["ITEMS"] as $itemName=>$arItems):
    $arFields = Array(
...
foreach($arResult["COLLECTIONS"] as $key=>$arItem):
    ...
    foreach($arItem as $itemCode):
        $arOffer = CIBlockElement::GetList(Array(), Array("IBLOCK_ID"=>IB_OFFERS, "XML_ID"=>$itemCode, "!PROPERTY_CML2_LINK_VALUE"=>false), false, Array("nTopCount"=>1), Array("ID", "IBLOCK_ID", "PROPERTY_CML2_LINK"))->fetch();
