...
function itemCard($arItem) {
    ...
    $first = true;
    $category = $arItem["ITEM"]["PROPERTY_CATEGORY_XML_ID"] ? getCode($arItem["ITEM"]["PROPERTY_CATEGORY_XML_ID"]) : "empty";
                                    ...
                                    <select class="offer_size"<?if($singleOffer || count($arItem["SIZES"]) === 1 ):?> disabled<?endif?>>
                                        ...
                                        <?else:
                                            foreach($arItem["SIZES"] as $size=>$arOffers):?>
                                                 <option value="<?=$size?>"<?if(in_array($offerId, $arOffers)):?> selected<?endif?>><?=$size?></option>
                                            <?endforeach;
                                        endif?>
                                    </select>
                                </div>
                        ...
                        <p>
                            ...
                            <?else:
                                ...
                                foreach($arItem["COLORS"][$arOffer["SIZE"]] as $colorId=>$arColor):
                                    if(!$arItem["COLORS_OFFERS"][$arOffer["SIZE"]][$colorId])
                                        continue;
